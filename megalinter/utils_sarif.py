...
def normalize_sarif_files(linter: Linter):
    ...
    if linter.sarif_output_file is not None and os.path.isfile(
        ...
        if load_ok is True:
            ...
            if config.get("SARIF_REPORTER_NORMALIZE_LINTERS_OUTPUT", True) == 'true' and default_workspace:
                clear_default_workspace_prefix(linter.sarif_output_file, default_workspace) 
